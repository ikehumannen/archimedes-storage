...
string getString(string record, int &pos1, string delim)
{
    ...
    temp.erase(0,1);                            // remove first quotation
    temp.erase(temp.size()-1);                  // remove second quotation
...
void compareStates(vector<Population> &states, vector<int> &twoStates, vector<string> stateAbbr, int  vSize)
{
    ...
    {
        cin >> abbr;
        for (int s = 0; s < abbr.size(); s++)        //check each character entered in abbr individually
            if (isalpha(abbr[s]) == 0)              //is false when abbr has a non-letter in it
                clearInput(abbr);                   //clear abbr

        for (int i = 0; i < abbr.size(); i++) abbr[i] = toupper(abbr[i]);   //make all letters upper case to prevent problems
...
void getDataSet(ifstream &inFile, vector<Population> &states)
{
    ...
    int loopCount;                             //track how many times program has repeated, prevents problems with empty line in file
...
do
{
    ...
    switch(menuItem)
    {
        ...
        case 2:                                   //total population for 2018
            {
